...
def normalize_list(lst, method, normalize_threshold):
...
def analysis_data(model, P, normalize_threshold):
    unit = 10 ** 9  # 固定 unit 为 1e9
    P = P * 1e9
    ...
    method = 'SMI'
    excel_save_path = f"analysis/tools/hyperparameter_experiments/2_normalize/tables/{model}_{normalize_threshold}.xlsx"
    figure_save_path = f"analysis/tools/hyperparameter_experiments/2_normalize/figures/{model}_{normalize_threshold}.pdf"

    mode = 'mi'
    metrics = [js[mode] for js in data]
    metrics = normalize_list(metrics, method, normalize_threshold)
...
if __name__ == "__main__":
    base_path = "analysis/tools/hyperparameter_experiments/2_normalize"
    if not os.path.exists(f"{base_path}/figures"):
        os.mkdir(f"{base_path}/figures")
    if not os.path.exists(f"{base_path}/tables"):
        os.mkdir(f"{base_path}/tables")
    ...
    for normalize_threshold in normalize_thresholds:
        for i in range(len(df)):
            model = df.loc[i, 'models'].split('/')[-1]
            P = df.loc[i, 'model_size']
            # 异步执行analysis_data函数
            result = pool.apply_async(analysis_data, args=(model, P, normalize_threshold))
