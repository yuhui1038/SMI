...
def search_multipro(part, file):
    ...
    save_path = f'search/slimpajama/outputs/part_{part}.json'
    ...
    with open(file, 'r', encoding='utf-8') as f:
        for line in tqdm(f):
            js = json.loads(line)
            if js['meta']['redpajama_set_name'] == 'RedPajamaGithub':
                continue
...
if __name__ == '__main__':
    files = sorted(glob("/mnt/data_26/pretraining_data/SlimPajama-627B/train/**/*.jsonl", recursive=True))
    print(len(files))
