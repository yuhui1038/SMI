...
def analysis_data(model, P):
    ...
    for j in range(3):
        ...
        figure_save_path = f"analysis/main_figures/{model}_{method}.pdf"
        if method == 'CO-OCCUR':
            mode = 'time_s1j1'
        else:
            mode = 'mi'
        ...
        results.loc[j, 'model'] = model
        results.loc[j, 'method'] = method
        results.loc[j, 'a'] = slope
        results.loc[j, 'b'] = intercept
        results.loc[j, 'R2'] = r2
        ...
        results.loc[j, 'x intercept'] = x_intercept

        # 计算 upper bound
        upper_bound = slope + intercept
        results.loc[j, 'upper bound'] = upper_bound
...
if __name__ == "__main__":
    if not os.path.exists("analysis/main_figures"):
        os.mkdir("analysis/main_figures")
    if not os.path.exists("analysis/tables"):
        os.mkdir("analysis/tables")
